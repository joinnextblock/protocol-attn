...
function App() {
  ...
  onMount(async () => {
    ...
    const subscription = await pool.subscribeMany(
      relays,
      filters,
      handlers,
    );
  ...
  const billboard_profile = from(
    queryStore.createQuery(ProfileQuery, BILLBOARD_PUBKEY)
  );
  ...
  const sortedAttentionEvents = () => {
    ...
    return events.sort((a, b) => {
      const parsed_a = a.tags.find(tag => tag[0] === 'max_duration')?.[1] * a.tags.find(tag => tag[0] === 'sats_per_second')?.[1];
      const parsed_b = b.tags.find(tag => tag[0] === 'max_duration')?.[1] * b.tags.find(tag => tag[0] === 'sats_per_second')?.[1];
  ...
  const sortedPromotionEvents = () => {
    ...
    return events.sort((a, b) => {
      const parsed_a = a.tags.find(tag => tag[0] === 'duration')?.[1] * a.tags.find(tag => tag[0] === 'sats_per_second')?.[1];
      const parsed_b = b.tags.find(tag => tag[0] === 'duration')?.[1] * b.tags.find(tag => tag[0] === 'sats_per_second')?.[1];
  ...
  const total_attention_duration = () => attention_events()?.reduce((sum, event) => {
    const max_duration = parseInt(event.tags.find(tag => tag[0] === 'max_duration')?.[1]);
    return sum + max_duration;
  }, 0);

  const total_promotion_duration = () => promotion_events()?.reduce((sum, event) => {
          ...
          <div class="card">
            <div>
              <For each={sortedAttentionEvents()}>
                {(attention_event) => (
                  <Attention attention_event={attention_event} />
                )}
              </For>
            </div>
          </div>
          ...
          <div class="card">
            <div>
              <For each={sortedPromotionEvents()}>
                {(promotion_event) => (
                  <Promotion promotion_event={promotion_event} />
                )}
              </For>
            </div>
          </div>
