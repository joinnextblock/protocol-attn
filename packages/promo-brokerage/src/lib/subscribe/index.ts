...
export const subscribe = ({ relay_handler, key_manager }: SubscribeArgs): SubscribeResult => {
  const attention_event_subscription = relay_handler.subscribeToRequests(
    async (attention_event: Event) => {
      ...
      const attention_max_duration = parseFloat(
        attention_event.tags.find((tag: any) => tag[0] === 'max_duration')?.[1] || '0'
      );
      const attention_sats_per_second = parseFloat(
        attention_event.tags.find((tag: any) => tag[0] === 'sats_per_second')?.[1] || '0'
      );
      ...
      for (const promotion_event of promotion_events) {
        ...
        if (promotion_billboard_pubkey === attention_billboard_pubkey) {
          const promotion_duration = parseFloat(
            promotion_event.tags.find((tag: any) => tag[0] === 'duration')?.[1] || '0'
          );
          const promotion_sats_per_second = parseFloat(
            promotion_event.tags.find((tag: any) => tag[0] === 'sats_per_second')?.[1] || '0'
          );
          ...
          if (promotion_sats_per_second >= attention_sats_per_second && promotion_duration <= attention_max_duration) {
            ...
            await publish_match_event(
              { attention_event, promotion_event, billboard_pubkey: attention_billboard_pubkey },
              { relay_handler, key_manager }
            );
          }
        }
      }
    ...
    async (promotion_event: Event) => {
      ...
      for (const attention_event of attention_events) {
        ...
        if (promotion_billboard_pubkey === attention_billboard_pubkey) {
          ...
          console.log(
            'promotion_sats_per_second',
            promotion_sats_per_second,
            'attention_sats_per_second',
            attention_sats_per_second
          );
          console.log('promotion_duration', promotion_duration, 'attention_max_duration', attention_max_duration);
          if (promotion_sats_per_second >= attention_sats_per_second && promotion_duration <= attention_max_duration) {
            ...
            await publish_match_event(
              { attention_event, promotion_event, billboard_pubkey: promotion_billboard_pubkey },
              { relay_handler, key_manager }
            );
          }
        }
  ...
  return [attention_event_subscription, promotion_event_subscription, match_event_subscription];
};
